(function(n){var l,f=/[-+,> ]/;n([],l=function(d,e){function a(e){function m(){s&&(t&&s!=t)&&(t==e&&(w||(w=f.test(x)&&d.createDocumentFragment()))||t).insertBefore(s,q||null)}for(var w,p,q,t,s,l=arguments,n=l[0],z=0;z<l.length;z++){var x=l[z];if("object"==typeof x){p=!1;if(x instanceof Array){s=d.createDocumentFragment();for(var y=0;y<x.length;y++)s.appendChild(a(x[y]));x=s}if(x.nodeType)s=x,m(),t=x,q=0;else for(y in x)s[y]=x[y]}else if(p)p=!1,s.appendChild(d.createTextNode(x));else{1>z&&(e=null);
p=!0;if(n=x.replace(c,function(c,f,w,p,n,y){f&&(m(),"-"==f||"+"==f?(t=(q=s||t).parentNode,s=null,"+"==f&&(q=q.nextSibling)):("<"==f?t=s=(s||t).parentNode:(","==f?t=e:s&&(t=s),s=null),q=0),s&&(t=s));if((c=!w&&p)||!s&&(w||n))"$"==c?(c=l[++z],t.appendChild(d.createTextNode(c))):(c=c||a.defaultTag,(f=v&&l[z+1]&&l[z+1].name)&&(c="<"+c+' name="'+f+'">'),s=h&&~(k=c.indexOf("|"))?d.createElementNS(h[c.slice(0,k)],c.slice(k+1)):d.createElement(c));if(w)if("$"==p&&(p=l[++z]),"#"==w)s.id=p;else if(f=(c=s.className)&&
(" "+c+" ").replace(" "+p+" "," "),"."==w)s.className=c?(f+p).substring(1):p;else if("!"==x){var B;v?a("div",s,"<").innerHTML="":(B=s.parentNode)&&B.removeChild(s)}else f=f.substring(1,f.length-1),f!=c&&(s.className=f);n&&("$"==y&&(y=l[++z]),"style"==n?s.style.cssText=y:(w="!"==n.charAt(0)?(n=n.substring(1))&&"removeAttribute":"setAttribute",y=""===y?n:y,h&&~(k=n.indexOf("|"))?s[w+"NS"](h[n.slice(0,k)],n.slice(k+1),y):s[w](n,y)));return""}))throw new SyntaxError("Unexpected char "+n+" in "+x);m();
t=n=s||t}}e&&w&&e.appendChild(w);return n}f=e||f;var c=/(?:\s*([-+ ,<>]))?\s*(\.|!\.?|#)?([-\w%$|]+)?(?:\[([^\]=]+)=?['"]?([^\]'"]*)['"]?\])?/g,k,h=!1;d=d||document;var v="object"==typeof d.createElement;a.addNamespace=function(c,a){d.createElementNS?(h||(h={}))[c]=a:d.namespaces.add(c,a)};a.defaultTag="div";a.forDocument=l;return a})})(function(n,l,f){f=f||l;"function"===typeof define?define([],function(){return f()}):"undefined"==typeof window?require("./node-html")(module,f):put=f()});
...
h[k]=Epoch.Util.defaults(a[k],c[k]):h[k]=a[k]:h[k]=null!=a[k]?a[k]:c[k];return h};Epoch.Util.formatSI=function(a,c,k){var h,v,d,e;null==c&&(c=1);null==k&&(k=!1);if(1E3>a){if((a|0)!==a||k)a=a.toFixed(c);return a}e="KMGTPEZY".split("");for(v in e)if(d=e[v],h=Math.pow(10,3*((v|0)+1)),a>=h&&a<Math.pow(10,3*((v|0)+2))){a/=h;if((a|0)!==a||k)a=a.toFixed(c);return""+a+" "+d}};Epoch.Util.formatBytes=function(a,c,k){var h,v,d,e;null==c&&(c=1);null==k&&(k=!1);if(1024>a)return""+a+" B";e="KB MB GB TB PB EB ZB YB".split(" ");
for(v in e)if(d=e[v],h=Math.pow(1024,(v|0)+1),a>=h&&a<Math.pow(1024,(v|0)+2)){a/=h;if((a|0)!==a||k)a=a.toFixed(c);return""+a+" "+d}};Epoch.Util.dasherize=function(a){return a.replace("\n","").replace(/\s+/,"-").toLowerCase()};Epoch.Util.domain=function(a,c){var k,h,v,d,e,f,p,q;null==c&&(c="x");v={};d=0;for(f=a.length;d<f;d++)for(k=a[d],q=k.values,e=0,p=q.length;e<p;e++)k=q[e],v[k[c]]=!0;d=[];for(h in v)d.push(h);return d};Epoch.toRGBA=function(a,c){var k,h;if(a.match(/^rgba\(\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\)/))k=
...
a){var h;if(null!=a)return null==(h=this._events)[c]&&(h[c]=[]),this._events[c].push(a)};a.prototype.off=function(c,a){var h,v;if(Epoch.isObject(this._events[c])){if(null==a)return delete this._events[c];for(v=[];0<=(h=this._events[c].indexOf(a));)v.push(this._events[c].splice(h,1));return v}};a.prototype.trigger=function(c){var a,h,v,d,e,f,p,q;if(null!=this._events[c]){a=function(){var c,a,k;k=[];d=c=1;for(a=arguments.length;1<=a?c<a:c>a;d=1<=a?++c:--c)k.push(arguments[d]);return k}.apply(this,arguments);
p=this._events[c];q=[];e=0;for(f=p.length;e<f;e++)h=p[e],v=null,Epoch.isString(h)?v=this[h]:Epoch.isFunction(h)&&(v=h),null==v&&Epoch.exception("Callback for event '"+c+"' is not a function or reference to a method."),q.push(v.apply(this,a));return q}};return a}();Epoch.Chart.Base=function(a){function c(a){this.options=a;c.__super__.constructor.call(this);this.setData(this.options.data||[]);null!=this.options.el&&(this.el=$(this.options.el));this.width=this.options.width;this.height=this.options.height;
...
(function(){var n,l={}.hasOwnProperty,f=function(d,e){function a(){this.constructor=d}for(var c in e)l.call(e,c)&&(d[c]=e[c]);a.prototype=e.prototype;d.prototype=new a;d.__super__=e.prototype;return d};Epoch.Time.Plot=function(d){function e(c){var k,h,d,f,m=this;this.options=c;c=Epoch.Util.copy(this.options.margins)||{};e.__super__.constructor.call(this,this.options=Epoch.Util.defaults(this.options,a));this._queueSize=this.options.queueSize;this._queue=[];this.margins={};f=["top","right","bottom",
...
width:""+this.innerWidth()/this.pixelRatio+"px",height:""+this.innerHeight()/this.pixelRatio+"px",top:""+this.margins.top+"px",left:""+this.margins.left+"px","z-index":"999"});this.animation={interval:null,active:!1,delta:-(this.w()/this.options.fps),tickDelta:-(this.w()/this.pixelRatio/this.options.fps),frame:0,duration:this.options.fps};this._prepareTimeAxes();this._prepareRangeAxes();this.animationCallback=function(){return m._animate()}}var a;f(e,d);a={fps:24,historySize:120,windowSize:40,queueSize:10,
...
(c=this._ticks.shift(),null!=c.topEl&&c.topEl.remove(),null!=c.bottomEl))return c.bottomEl.remove()};e.prototype._updateTimeAxes=function(){var c,a,d,e,f,m,w;if(this.hasAxis("top")||this.hasAxis("bottom")){c=[this.animation.tickDelta,1/this.options.fps];a=c[0];c=c[1];m=this._ticks;w=[];e=0;for(f=m.length;e<f;e++)d=m[e],d.x+=a,this.hasAxis("bottom")&&d.bottomEl.attr("transform","translate("+(d.x+1)+",0)"),this.hasAxis("top")&&d.topEl.attr("transform","translate("+(d.x+1)+",0)"),d.enter?d.opacity+=
c:d.exit&&(d.opacity-=c),d.enter||d.exit?(this.hasAxis("bottom")&&d.bottomEl.css("opacity",d.opacity),this.hasAxis("top")?w.push(d.topEl.css("opacity",d.opacity)):w.push(void 0)):w.push(void 0);return w}};e.prototype.draw=function(a){};return e}(Epoch.Chart.Canvas);Epoch.Time.Stack=function(d){function e(){return n=e.__super__.constructor.apply(this,arguments)}f(e,d);e.prototype._prepareLayers=function(a){var c,d,e,f;e=d=0;for(f=a.length;e<f;e++)c=a[e],c.y0=d,d+=c.y;return a};e.prototype.setData=
function(a){var c,d,h,f,u;e.__super__.setData.call(this,a);u=[];c=a=0;for(f=this.data[0].values.length;0<=f?a<f:a>f;c=0<=f?++a:--a)h=0,u.push(function(){var a,e,f,v;f=this.data;v=[];a=0;for(e=f.length;a<e;a++)d=f[a],d.values[c].y0=h,v.push(h+=d.values[c].y);return v}.call(this));return u};e.prototype.extent=function(){var a,c,d,e,f,u,m,w;a=f=d=0;for(m=this.data[0].values.length;0<=m?f<m:f>m;a=0<=m?++f:--f){c=u=e=0;for(w=this.data.length;0<=w?u<w:u>w;c=0<=w?++u:--u)e+=this.data[c].values[a].y;e>d&&
...
a.stroke);if(null!=a["stroke-width"])return this.ctx.lineWidth=a["stroke-width"].replace("px","")};e.prototype._drawAreas=function(a){var c,d,e,f,u,m,w,p,q,t;null==a&&(a=0);m=[this.y(),this.w()];w=m[0];m=m[1];t=[];for(d=p=q=this.data.length-1;0>=q?0>=p:0<=p;d=0>=q?++p:--p){f=this.data[d];this.setStyles(f);this.ctx.beginPath();u=[this.options.windowSize,f.values.length,this.inTransition()];d=u[0];e=u[1];for(u=u[2];-2<=--d&&0<=--e;)c=f.values[e],c=[(d+1)*m+a,w(c.y+c.y0)],u&&(c[0]+=m),d===this.options.windowSize-
1?this.ctx.moveTo.apply(this.ctx,c):this.ctx.lineTo.apply(this.ctx,c);d=u?(d+3)*m+a:(d+2)*m+a;this.ctx.lineTo(d,this.innerHeight());this.ctx.lineTo(this.width*this.pixelRatio+m+a,this.innerHeight());this.ctx.closePath();t.push(this.ctx.fill())}return t};e.prototype._drawStrokes=function(a){var c,d,e,f,u,m,w,p,q,t;null==a&&(a=0);d=[this.y(),this.w()];w=d[0];m=d[1];t=[];for(d=p=q=this.data.length-1;0>=q?0>=p:0<=p;d=0>=q?++p:--p){f=this.data[d];this.setStyles(f);this.ctx.beginPath();u=[this.options.windowSize,
f.values.length,this.inTransition()];d=u[0];e=u[1];for(u=u[2];-2<=--d&&0<=--e;)c=f.values[e],c=[(d+1)*m+a,w(c.y+c.y0)],u&&(c[0]+=m),d===this.options.windowSize-1?this.ctx.moveTo.apply(this.ctx,c):this.ctx.lineTo.apply(this.ctx,c);t.push(this.ctx.stroke())}return t};e.prototype.draw=function(a){null==a&&(a=0);if(this.isVisible())return this.clear(),this._drawAreas(a),this._drawStrokes(a)};return e}(Epoch.Time.Stack)}).call(this);
...
null==a.stroke||"none"===a.stroke?"transparent":a.stroke;if(null!=a["stroke-width"])return this.ctx.lineWidth=a["stroke-width"].replace("px","")};e.prototype.draw=function(a){var c,d,e,f,u,m,w,p,q,t,s,n,l,z,x,y,C;null==a&&(a=0);if(this.isVisible()){this.clear();l=[this.y(),this.w()];n=l[0];s=l[1];x=this.data;C=[];l=0;for(z=x.length;l<z;l++)q=x[l],0<q.values.length&&(this.setStyles(q.className),y=[this.options.windowSize,q.values.length,this.inTransition()],m=y[0],p=y[1],w=(t=y[2])?-1:0,C.push(function(){var l,
x;for(x=[];--m>=w&&0<=--p;)d=q.values[p],l=[m*s+a,d.y,d.y0],e=l[0],f=l[1],u=l[2],t&&(e+=s),c=[e+1,n(f+u),s-2,this.innerHeight()-n(f)+0.5*this.pixelRatio],this.ctx.fillRect.apply(this.ctx,c),x.push(this.ctx.strokeRect.apply(this.ctx,c));return x}.call(this)));return C}};return e}(Epoch.Time.Stack)}).call(this);
...
function(a){a=this.getStyles(a);this.ctx.fillStyle=a.fill;this.ctx.strokeStyle=a.stroke;if(null!=a["stroke-width"])return this.ctx.lineWidth=a["stroke-width"].replace("px","")};d.prototype.draw=function(){var a,c,d,e,f,u,m,l,p,q,n,s;if(this.isVisible()){f=[this.centerX(),this.centerY(),this.radius()];d=f[0];e=f[1];f=f[2];m=[this.options.tickOffset,this.options.tickSize];l=m[0];p=m[1];this.clear();m=d3.scale.linear().domain([0,this.options.ticks]).range([-1.125*Math.PI,Math.PI/8]);this.setStyles(".epoch .gauge .tick");
this.ctx.beginPath();a=n=0;for(s=this.options.ticks;0<=s?n<=s:n>=s;a=0<=s?++n:--n)a=m(a),a=[Math.cos(a),Math.sin(a)],c=a[0],u=a[1],a=c*(f-l)+d,q=u*(f-l)+e,c=c*(f-l-p)+d,u=u*(f-l-p)+e,this.ctx.moveTo(a,q),this.ctx.lineTo(c,u);this.ctx.stroke();this.setStyles(".epoch .gauge .arc.outer");this.ctx.beginPath();this.ctx.arc(d,e,f,-1.125*Math.PI,0.125*Math.PI,!1);this.ctx.stroke();this.setStyles(".epoch .gauge .arc.inner");this.ctx.beginPath();this.ctx.arc(d,e,f-10,-1.125*Math.PI,0.125*Math.PI,!1);this.ctx.stroke();
...
d,5)}};d.prototype.setData=function(a){var e,f,v,u,m=this;d.__super__.setData.call(this,a);v=this.data;u=[];e=0;for(f=v.length;e<f;e++)a=v[e],u.push(a.values=a.values.map(function(a){return m._prepareEntry(a)}));return u};d.prototype._prepareEntry=function(a){var d,e,f,u,m,l,p,q,n,s;m={time:a.time,max:0,buckets:{}};f=this.options.bucketRange;u=f[0];f=f[1];l=(f-u)/this.options.buckets;e=d=0;for(q=this.options.buckets;0<=q?d<q:d>q;e=0<=q?++d:--d)m.buckets[u+l*(e+1)]=0;n=a.histogram;for(p in n)for(a=
n[p],e=q=0,s=this.options.buckets;0<=s?q<s:q>s;e=0<=s?++q:--q)if(d=u+l*(e+1),p<d||e===this.options.buckets-1){m.buckets[d]+=a;break}u=m.buckets;for(f in u)a=u[f],m.max=Math.max(m.max,a);return m};d.prototype.y=function(){return d3.scale.linear().domain(this.options.bucketRange).range([this.innerHeight(),0])};d.prototype.ySvg=function(){return d3.scale.linear().domain(this.options.bucketRange).range([this.innerHeight()/this.pixelRatio,0])};d.prototype.h=function(){return this.innerHeight()/this.options.buckets};
...
a=this.options.windowSize;for(e=[];0<=--d&&0<=--a;)e.push(this._paintEntry(d,a));return e};d.prototype._paintEntry=function(a,d){var e,f,l,m,n,p,q,t,s,A,B,z,x,y,C;null==a&&(a=null);null==d&&(d=null);f=[this.w(),this.h()];z=f[0];q=f[1];null==a&&(a=this.data[0].values.length-1);null==d&&(d=this.options.windowSize);n=[];f={};B=0;A=this.data;l=0;for(t=A.length;l<t;l++){s=A[l];p=s.values[a];x=p.buckets;for(e in x)m=x[e],null==f[e]&&(f[e]=0),f[e]+=m;B+=p.max;m=this.getStyles("."+s.className.split(" ").join(".")+
" rect.bucket");p.color=m.fill;n.push(p)}s=d*z;this.p.clearRect(s,0,z,this.paintHeight);t=this.options.buckets;C=[];for(e in f){m=f[e];l=this._avgLab(n,e);x=A=0;for(y=n.length;x<y;x++)p=n[x],A+=p.buckets[e]/m*B;0<m&&(this.p.fillStyle=Epoch.toRGBA(l,this._colorFn(m,A)),this.p.fillRect(s,(t-1)*q,z-this.options.bucketPadding,q-this.options.bucketPadding));C.push(t--)}return C};d.prototype._shiftPaintCanvas=function(){var a;a=this.p.getImageData(this.w(),0,this.paintWidth-this.w(),this.paintHeight);return this.p.putImageData(a,
0,0)};d.prototype._avgLab=function(a,d){var e,f,l,m,n,p,q,t;q=[0,0,0,0];n=q[0];e=q[1];f=q[2];q=q[3];p=0;for(t=a.length;p<t;p++)l=a[p],null!=l.buckets[d]&&(q+=l.buckets[d]);for(m in a)l=a[m],p=null!=l.buckets[d]?l.buckets[d]|0:0,p/=q,l=d3.lab(l.color),n+=p*l.l,e+=p*l.a,f+=p*l.b;return d3.lab(n,e,f).toString()};d.prototype.draw=function(a){null==a&&(a=0);if(this.isVisible())return this.clear(),this.ctx.drawImage(this.paint,a,0)};return d}(Epoch.Time.Plot)}).call(this);
...
"")};e.prototype.draw=function(a){var c,d,e,f,l,m,n,p,q,t,s;null==a&&(a=0);this.clear();d=[this.y(),this.w()];n=d[0];m=d[1];t=this.data;s=[];p=0;for(q=t.length;p<q;p++)if(f=t[p],0<f.values.length){this.setStyles(f.className);this.ctx.beginPath();l=[this.options.windowSize,f.values.length,this.inTransition()];d=l[0];e=l[1];for(l=l[2];-2<=--d&&0<=--e;)c=f.values[e],c=[(d+1)*m+a,n(c.y)],l&&(c[0]+=m),d===this.options.windowSize-1?this.ctx.moveTo.apply(this.ctx,c):this.ctx.lineTo.apply(this.ctx,c);s.push(this.ctx.stroke())}return s};
